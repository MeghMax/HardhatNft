...
const { assert, expect, emit } = require("chai")
...
!developmentChains.includes(network.name)
    ...
    : describe("Random IPFS NFT Unit Tests", function () {
          ...
          describe("Get Dog Breed From ModdedRNG", () => {
              it("Should return pug if moddedRng <10", async function () {
                  const expectedValue = await randomIpfsNft.getBreedFromModdedRng(7)
                  assert.equal(0, expectedValue)
              })
              it("Shoyld return shiba-inu if moddedRng is between 10-39", async () => {
              ...
              it("Should revert if moddedRng is > 99", async () => {
                  expect(randomIpfsNft.getBreedFromModdedRng(100)).to.be.revertedWith(
